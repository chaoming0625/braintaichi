...
class EventJitFPHomoLinear(Layer):
  ...
  def update(self, x):
    if x.ndim == 1:
      if self.method is None:
        event_jitconn_mv = bm.jitconn.event_mv_prob_homo_taichi
      else:
        event_jitconn_mv = bm.jitconn.event_mv_prob_homo
      return event_jitconn_mv(x, self.weight, self.prob, self.seed,
                                           shape=(self.num_out, self.num_in),
                                           transpose=self.transpose,
                                           outdim_parallel=not self.atomic)
    elif x.ndim == 2:
...
class EventJitFPUniformLinear(Layer):
  ...
  def update(self, x):
    if x.ndim == 1:
      if self.method is None:
        event_jitconn_mv = bm.jitconn.event_mv_prob_uniform_taichi
      else:
        event_jitconn_mv = bm.jitconn.event_mv_prob_uniform
      return event_jitconn_mv(x, self.w_low, self.w_high, self.prob, self.seed,
                                              shape=(self.num_out, self.num_in),
                                              transpose=self.transpose,
                                              outdim_parallel=not self.atomic)
    elif x.ndim == 2:
...
class EventJitFPNormalLinear(Layer):
  ...
  def update(self, x):
    if x.ndim == 1:
      if self.method is None:
        event_jitconn_mv = bm.jitconn.event_mv_prob_normal_taichi
      else:
        event_jitconn_mv = bm.jitconn.event_mv_prob_normal
      return event_jitconn_mv(x, self.w_mu, self.w_sigma, self.prob, self.seed,
                                             shape=(self.num_out, self.num_in),
                                             transpose=self.transpose,
                                             outdim_parallel=not self.atomic)
    elif x.ndim == 2:
