...
ti_float = ti.float32 if not jax.config.read('jax_enable_x64') else ti.float64
ti_int = ti.int32 if not jax.config.read('jax_enable_x64') else ti.int64


def _change_precision(precision: int):
  global ti_float, ti_int
  if precision == 64:
    ti_float = ti.float64
    ti_int = ti.int64
  elif precision == 32:
    ...
    ti_float = ti.bfloat16
    ti_int = ti.int16
  else:
    raise ValueError(f'Unsupported precision: {precision}')


bst.environ.register_default_behavior('precision', _change_precision)
...
def lfsr88_randn(key: ti.types.vector(4, ti.u32), epsilon=1e-10):
  ...
  u1 = ti.cast(ti.max(u1, epsilon), ti_float)

  # Normalize the uniform samples
  mag = ti.cast(ti.sqrt(-2.0 * ti.log(u1)), ti_float)

  # Box-Muller transform
  # z1 = mag * ti.cos(2 * ti.math.pi * u2)
  z2 = ti.cast(mag * ti.sin(2 * ti.math.pi * u2), ti_float)
...
def lfsr88_random_integers(key: ti.types.vector(4, ti.u32), low, high):
  ...
  return key, ti.cast((key[0] ^ key[1] ^ key[2]) % (high + 1 - low) + low, ti_int)
...
def lfsr88_uniform(key: ti.types.vector(4, ti.u32), low, high):
  ...
  """
  key = lfsr88_next_key(key)
  r = (key[0] ^ key[1] ^ key[2]) * ti.cast(2.3283064365386963e-10, ti_float)
  return key, ti.cast(r * (high - low) + low, ti_float)
...
def lfsr88_rand(key: ti.types.vector(4, ti.u32)):
  ...
  """
  key = lfsr88_next_key(key)
  return key, (key[0] ^ key[1] ^ key[2]) * ti.cast(2.3283064365386963e-10, ti_float)
...
def lfsr113_normal(key: ti.types.vector(4, ti.u32), mu, sigma, epsilon=1e-10):
  ...
  return key, ti.cast(mu + sigma * r, ti_float)
...
def lfsr113_random_integers(key: ti.types.vector(4, ti.u32), low, high):
  ...
  return key, ti.cast((key[0] ^ key[1] ^ key[2] ^ key[3]) % (high + 1 - low) + low, ti_int)


@ti.func
def lfsr113_randint(key: ti.types.vector(4, ti.u32)):
  key = lfsr113_next_key(key)
  return key, ti.cast(key[0] ^ key[1] ^ key[2] ^ key[3], ti_int)
...
def lfsr113_uniform(key: ti.types.vector(4, ti.u32), low, high):
  ...
  """
  key = lfsr88_next_key(key)
  r = (key[0] ^ key[1] ^ key[2] ^ key[3]) * ti.cast(2.3283064365386963e-10, ti_float)
  return key, ti.cast(r * (high - low) + low, ti_float)
...
def lfsr113_rand(key: ti.types.vector(4, ti.u32)):
  ...
  """
  key = lfsr113_next_key(key)
  return key, (key[0] ^ key[1] ^ key[2] ^ key[3]) * ti.cast(2.3283064365386963e-10, ti_float)
