...
def _ti_float():
    return ti.float64 if jax.config.read('jax_enable_x64') else ti.float32


def _ti_int():
    return ti.int64 if jax.config.read('jax_enable_x64') else ti.int32
...
def lfsr88_randn(key: ti.types.vector(4, ti.u32), epsilon=1e-10):
    ...
    u1 = ti.cast(ti.max(u1, epsilon), _ti_float())

    # Normalize the uniform samples
    mag = ti.cast(ti.sqrt(-2.0 * ti.log(u1)), _ti_float())

    # Box-Muller transform
    # z1 = mag * ti.cos(2 * ti.math.pi * u2)
    z2 = ti.cast(mag * ti.sin(2 * ti.math.pi * u2), _ti_float())
...
def lfsr88_random_integers(key: ti.types.vector(4, ti.u32), low, high):
    ...
    return key, ti.cast((key[0] ^ key[1] ^ key[2]) % (high + 1 - low) + low, _ti_int())
...
def lfsr88_uniform(key: ti.types.vector(4, ti.u32), low, high):
    ...
    """
    key = lfsr88_next_key(key)
    r = (key[0] ^ key[1] ^ key[2]) * ti.cast(2.3283064365386963e-10, _ti_float())
    return key, ti.cast(r * (high - low) + low, _ti_float())
...
def lfsr88_rand(key: ti.types.vector(4, ti.u32)):
    ...
    """
    key = lfsr88_next_key(key)
    return key, (key[0] ^ key[1] ^ key[2]) * ti.cast(2.3283064365386963e-10, _ti_float())
...
def lfsr113_normal(key: ti.types.vector(4, ti.u32), mu, sigma, epsilon=1e-10):
    ...
    return key, ti.cast(mu + sigma * r, _ti_float())
...
def lfsr113_uniform(key: ti.types.vector(4, ti.u32), low, high):
    ...
    """
    key = lfsr88_next_key(key)
    r = (key[0] ^ key[1] ^ key[2] ^ key[3]) * ti.cast(2.3283064365386963e-10, _ti_float())
    return key, ti.cast(r * (high - low) + low, _ti_float())
...
def lfsr113_rand(key: ti.types.vector(4, ti.u32)):
    ...
    """
    key = lfsr113_next_key(key)
    return key, (key[0] ^ key[1] ^ key[2] ^ key[3]) * ti.cast(2.3283064365386963e-10, _ti_float())
